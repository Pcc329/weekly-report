...
1.本週三條工作線並行推進：主線為「資料庫遷移收尾」，兩條重要分支為「資料庫升級評估報告（主任交辦）」與「PM 開案方法論教程開案（組長交辦）」。
...
5.**PM 開案方法論教程完成 kick-off**：依組長交辦與同事討論結果正式開案，教學簡報與 Kit 架構已交付；下一步進入「請顧問同仁實際試用、驗證教程可用性」階段。
...
### 分支三：PM 開案方法論教程（組長交辦 + 同事討論開案）

| 任務 | 狀態 | 說明 |
|---|---|---|
| 正式開案（kick-off） | ✅ 完成 | 依組長交辦與同事討論結果開案 |
| 方法論 Kit 交付 | ✅ 完成 | `core/`（跨專案通用）＋ `instances/`（專案專屬）兩層架構，含開案訪談、AI 協作角色、GitHub/Vercel 教學等文件 |
...
### 5-1 累計支出（補正至 2026-07-30）

本週無新增訂閱、儲值或一次性採購入帳，累計金額與上週相同，維持 **NT$ 20,748**。依「用途」而非「廠商」分為四類：

| 類別 | 包含項目 | 金額 | 占比 | 性質 | 未來趨勢 |
|---|---|---:|---:|---|---|
| 資料庫基礎設施 | Airtable（未來：Supabase／CDN／Vercel） | NT$ 11,289 | 54.4% | 月費 | 遷移後可望下降 |
| 開發費用 | Codex + Claude 訂閱 | NT$ 4,750 | 22.9% | 建置期月費 | 成熟後可調降 |
| 資料來源採購 | Crunchbase | NT$ 3,150 | 15.2% | 一次性 | 視需求，非經常 |
| AI 推論（營運） | Claude API Credit | NT$ 1,559 | 7.5% | 用量計費 | 隨使用人數成長 |
| **累計合計** | | **NT$ 20,748** | 100% | | |

> 分類原則：同一家 Claude 拆成兩筆——訂閱屬「開發費用」（建置期成本、會趨於平緩），API 用量屬「AI 推論／營運」（隨使用人數成長）。逐項明細與 18 個月費用預估詳見[資料庫升級評估報告](https://pcc329.github.io/weekly-report/db_upgrade_roadmap_20260730.html)。
...
| 🟡 | PM 教程可用性驗證 | 請顧問同仁實際試用開案教程，收集回饋驗證是否 work |
